...
I haven't completely tested all of the problems, but as I use the basic features of vestal_versions, I will correct any errors I find and push the changes up here.

== Compatibility

I'm currently using it with Rails 4.2, Ruby 2.2.0.
...
* Storing which user is responsible for a revision. Rather than introduce a lot of controller magic to guess what to store, you can simply update an additional attribute on your versioned model: <tt>updated_by</tt>. The original method was to include the <tt>udpated_by</tt> attribute in the update, but for some reason, this doesn't seem to work when updating attributes in the web server. It definitely works in the console, but not in the controller. However, if the attribute is assigned before the update_attributes, it gets passed through properly.

    @user.updated_by = "Tyler"
    @user.update_attributes(:last_name => "Jobs")
